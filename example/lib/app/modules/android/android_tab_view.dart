...
class AndroidTabView extends StatefulWidget {
  const AndroidTabView({Key? key}) : super(key: key);

  @override
  State<AndroidTabView> createState() => _AndroidTabViewState();
}

class _AndroidTabViewState extends State<AndroidTabView> {
  Map map = {};

  Future<void> getBatteryInfo() async {
    final result = await DeviceInfoX().getAndroid();
    if (result != null) {
      map = result.toMap();
    }
